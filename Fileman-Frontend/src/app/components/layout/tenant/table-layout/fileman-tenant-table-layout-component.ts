...
@Component({
  selector: 'fileman-tenant-table-layout',
  templateUrl: './fileman-tenant-table-layout-component.html',
  styleUrls: ['./fileman-tenant-table-layout-component.css']
})
export class TenantTableLayout extends LayoutTenantCommons {
  openPullDowns: Array<MatSelect> = new Array<MatSelect>();

  sort(event) {
    const sortList = this.viewedTenants;
    if (event.sortType === SortType.ASC) {
       this.viewedTenants = sortList.sort((dataObject1, dataObject2) => {
